...
4. **`/pr` command:** **push the branch to `origin` first** (so GitHub has the latest commits), then if a PR for the current branch already exists, run **`gh pr edit`** with **`--title`** and **`--body-file`** so the description matches the latest branch changes; otherwise run **`gh pr create`** with **`--body-file`**. Then **print the PR URL** from **`gh`** output (or **`gh pr view --json url`**).
